...
def get_vector_file(dataset: str, model: str, feature_dir: str) -> str:
    ...
    # Require train and test groups to exist, but allow additional groups like 'text'
    required = {"train", "test"}
    missing = required - keys
    if missing:
        missing_str = ", ".join(sorted(missing))
        raise KeyError(f"Missing features for the {missing_str} split(s)")
...
def main(cfg: DictConfig) -> None:
    ...
    try:
        ...
        logging.exception("Uncaught exception during al_train")
