...
# --------- Config ---------
BASE_DIR = "ALFM/logs/results/food101/dino_vit_L14"
# METHODS = [
#     "disagreement_C1",
#     "disagreement_C10",
#     "disagreement_C15",
...
def plot_metric(all_aggs: dict, metric_key: str, metric_label: str):
    ...
    for method, df in all_aggs.items():
        ...
        if std_col in df_sorted.columns and not df_sorted[std_col].isna().all():
            plt.fill_between(x, y - s, y + s, alpha=0.15)

    plt.xlabel("num_samples")
    plt.ylabel(metric_label)
    plt.title(f"{metric_label} vs num_samples (Food101, dino_vit_L14)")
    plt.grid(True, linestyle="--", linewidth=0.6, alpha=0.6)
    plt.legend(title="Method", ncol=2, fontsize=9)
    plt.tight_layout()
    out_png = os.path.join(OUT_PLOTS_DIR, f"compare_food101_{metric_label.lower()}.png")
    plt.savefig(out_png, dpi=200)
    out_pgf = os.path.join(OUT_PLOTS_DIR, f"compare_food101_{metric_label.lower()}.pgf")
