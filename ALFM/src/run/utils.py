...
class ExperimentLogger:
    ...
    def log_cfg(self, cfg: Dict[str, Any]) -> None:
        ...
        self.file_name = f"{cfg['query_strategy']['name']}_g14-{hash_str}"
    ...
    def log_scores(
        self, scores: Dict[str, float], iteration: int, num_iter: int, num_samples: int, extras: Dict[str, Any] | None = None
    ) -> None:
        logging.info(
        ...
        if extras:
            # extend fields in stable order
            for k in extras.keys():
                if k not in fields:
                    fields.append(k)
            data.update(extras)
        else:
            # keep columns consistent across runs if you prefer:
            pass
        csv_file = self.csv_dir / f"{self.file_name}.csv"
