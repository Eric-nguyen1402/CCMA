...
class ProbCover(BaseQuery):
    ...
    def query(self, num_samples: int) -> NDArray[np.bool_]:
        ...
        # --- FIX: Rebuild the graph state at the start of every query ---
        # 1. Build the full graph if it doesn't exist
        if not self.graph_constructed:
            self._estimate_delta(self.delta_iter)
            self._build_graph()
            self.graph_constructed = True
            # Keep a pristine copy of the full graph
            self.full_edge_list = self.edge_list.clone()
        
        # 2. Start each query with a fresh copy of the full graph
        self.edge_list = self.full_edge_list.clone()

        # 3. Remove all nodes covered by the CURRENT labeled pool
        ...
        for _ in track(range(num_samples), description="[green]Probcover query"):
            # Check if there are any edges left to select from
            if self.edge_list.shape[0] == 0:
                logging.warning("[ProbCover] Graph is empty, cannot select more samples.")
                break
