...
class Typiclust(BaseQuery):
    ...
    def _typical_vec_id(self, features: NDArray[np.float32], knn: int, gpu_res: faiss.StandardGpuResources) -> int:
    ...
    def _select_points(
        ...
        full_count = torch.bincount(clust_labels[labeled_pool])
        count = torch.zeros(max(cluster_ids.max() + 1, full_count.shape[0]), dtype=full_count.dtype)
        count[:full_count.shape[0]] = full_count
        count = count[cluster_ids]
        ...
        for i in track(range(num_samples), description="[green]Typiclust query"):
            ...
            if len(indices) > SUBSAMPLE_LIMIT:
                subsample_indices = torch.randperm(len(indices))[:SUBSAMPLE_LIMIT]
                vectors = features[indices[subsample_indices]]
            else:
                vectors = features[indices]

            if len(vectors) > self.min_size:
                knn = min(self.knn, len(indices) // 2)
                vec_id = self._typical_vec_id(vectors, knn, gpu_res)

                # Map the subsampled vec_id back to the original index
                if len(indices) > SUBSAMPLE_LIMIT:
                    original_index_in_cluster = subsample_indices[vec_id]
                else:
                    original_index_in_cluster = vec_id

                selected_global_index = indices[original_index_in_cluster]
                selected[selected_global_index] = True
                clust_labels[selected_global_index] = -1
